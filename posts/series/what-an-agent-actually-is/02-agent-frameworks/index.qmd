...
description: "The model emits, the API parses, your code dispatches and loops."
date: 2026-04-14
order: 2
draft: false
---
...
[The first article](/posts/series/what-an-agent-actually-is/01-what-is-an-agent/index.qmd) treated "agent" as a region on a spectrum. This one goes inside one specific point on that spectrum, the loop-with-tools agent, and shows what's actually happening when you call `Runner.run(agent, message)` or its equivalent.
...
The API surfaces this output in a typed `tool_calls` field, with `content: null` because the model didn't generate any text alongside.
