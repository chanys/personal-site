...
Rather than pick a winner, stop asking whether something is an agent. Ask how much autonomy it has, and whether that's the right amount for the task.
...
These definitions do not fully agree.
...
Each step adds capability and removes a constraint. Each one is also a deliberate engineering choice. More autonomy is different, not better. A task that would be well served by a fixed workflow doesn't improve when you add a planning loop, and a task that genuinely needs open-ended exploration isn't solved by a single tool call.
...
If you read AI marketing in 2026, the implicit definition of an agent is the right end of the spectrum: a system that operates without human input, makes decisions, takes actions, plans across long horizons. That's the picture, but it's a small slice of what's actually deployed.
